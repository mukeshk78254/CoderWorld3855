...
const CodingActivity = ({ submissionActivity = [], yearlyProgress = null, weeklyData = [], totalSubmissions = 0, streak = 0 }) => {
    ...
    // --- CORE DATA MAPPING ---
    // Strictly use fresh backend data
    const submissionMap = yearlyProgress?.heatmapData || {};
    const dayData = weeklyData; 
    const totalDaysActive = yearlyProgress?.totalDaysActive || Object.keys(submissionMap).length;

    // 2. Generate GitHub-style calendar grid and month mapping for year view
    const { calendarGrid, monthLabels } = useMemo(() => {
        ...
        // Find the first Sunday *before* or *on* Jan 1st of the selected year
        ...
        for (let week = 0; week < 53; week++) {
            const weekDays = [];
            let foundDayInSelectedYear = false;

            for (let day = 0; day < 7; day++) {
                const currentDate = new Date(startDate);
                currentDate.setDate(startDate.getDate() + (week * 7) + day);
                
                // Track the first column of each month
                if (currentDate.getMonth() !== currentMonth && currentDate.getFullYear() === selectedYear) {
                    currentMonth = currentDate.getMonth();
                    monthLabels.push({
                        month: currentDate.toLocaleDateString('en-US', { month: 'short' }),
                        // We use the week index for CSS translation
                        weekIndex: week 
                    });
                }

                if (currentDate.getFullYear() === selectedYear) {
                    foundDayInSelectedYear = true;
                }

                // Format: YYYY-MM-DD (must match backend heatmapData format)
                ...
                weekDays.push({
                    ...
                    isFuture: currentDate > today && currentDate.toDateString() !== today.toDateString(),
                    isSelectedYear: currentDate.getFullYear() === selectedYear
                });
            }
            // Only push weeks that contain at least one day in the selected year (or enough for the start)
            if (foundDayInSelectedYear || week < 5) {
               grid.push(weekDays);
            }
        }
        return { calendarGrid: grid, monthLabels };
                ...
                <div className="flex justify-between items-center mb-4">
                    <div>
                        ...
                        <div className="text-xs text-gray-400 mt-1 space-x-4">
                            <span>Active days: {totalDaysActive}</span>                                             
                            <span>Longest streak: {streak} days</span>
                            <span>Success rate: {totalSubmissions > 0 ? Math.round((totalDaysActive / 365) * 100) : 0}%</span>
                        </div>
                    </div>
                    ...
                    {viewMode === 'week' ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-6">
                                <div className="bg-gray-700/30 rounded-lg p-4 border border-gray-600/50">
                                    <div className="text-center">
                                        ...
                                        <div className="text-xs text-gray-400">Total Submissions (7 Days)</div>
                                    </div>
                                </div>
                                <div className="bg-gray-700/30 rounded-lg p-4 border border-gray-600/50">
                                    <div className="text-center">
                            ...
                            <div className="grid grid-cols-7 gap-2">
                                {dayData.map((day) => {
                                    // Use date key from backend, format for display
                                    ...
                                    const tooltipText = `${dayName}, ${monthName} ${dayNumber}: ${day.submissions} submission${day.submissions !== 1 ? 's' : ''}`;
                        ...
                        <div className="space-y-2">
                            <div className="flex gap-1 overflow-x-auto p-2">
                                ...
                                <div className="flex-grow">
                                    ...
                                    <div className="flex justify-start text-xs text-gray-400 mb-2 h-4 overflow-visible">
                                        {calendarGrid.map((week, weekIndex) => {
                                            const monthInfo = monthLabels.find(label => label.weekIndex === weekIndex);
                                            
                                            // Render month label only at the start of the month column
                                            if (monthInfo) {
                                                return (
                                                    <span 
                                                        key={monthInfo.month} 
                                                        className="absolute"
                                                        // 16px (w-4) + 2px (gap-1.5) = 17.5px total width per column. Using 18px for Tailwind gap approximations.
                                                        style={{ transform: `translateX(calc(${weekIndex * 18}px))` }} 
                                                    >
                                                        {monthInfo.month}
                                                    </span>
                                                );
